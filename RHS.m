...
%rate of change of E
dy(:,2) = trans - y(:,2)/p.w + mov'*y(:,2) - (mov)*y(:,2);
%rate of change of A
dy(:,3) = (1-p.p)*y(:,2)/p.w - p.r_1*y(:,3)/p.l_1 - (1-p.r_1)*y(:,3)/p.g_1 + mov'*y(:,3) - (mov)*y(:,3);
%rate of change of I1
dy(:,4) = p.p*y(:,2)/p.w - p.r_1*y(:,4)/p.l_1 - (1-p.r_1)*y(:,4)/p.g_1;
